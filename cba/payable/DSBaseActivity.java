...
public abstract class DSBaseActivity extends GenericActivity{
	
	private static final String LOG_TAG = "payable " + DSBaseActivity.class.getSimpleName();
	...
	private void registerBT_BC_Receiver(){
		Log.d(LOG_TAG, "registerBT_BC_Receiver: ");
	...
	protected void batteryLeavelThread(){
		Log.d(LOG_TAG, "batteryLeavelThread: ");
		Thread thread = new Thread(new Runnable(){
			public void run(){
				try {
					Thread.sleep(100) ;
	...
	protected void refereshConnection(){
		Log.d(LOG_TAG, "refereshConnection: ");
		registerBT_BC_Receiver() ;
		//_connectBTDevice() ;
		Thread thread = new Thread(new Runnable(){
			public void run(){
	...
	private void _connectBTDevice(){
		Log.d(LOG_TAG, "_connectBTDevice: ");
	...
	public boolean isReaderConnected(){
		Log.d(LOG_TAG, "isReaderConnected: ");
	...
	private final BroadcastReceiver stateChangeReceiver = new BroadcastReceiver(){
		...
		public void onReceive(Context context, Intent intent){
			...
			Log.i("PAY", "Binded device name : " + deviceName) ;
				Log.i("PAY", "BT name : " + device.getName()) ;
